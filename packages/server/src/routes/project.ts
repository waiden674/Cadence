...
router.post('/:projectId/brainstorm', async (req, res) => {
  ...
  res.json({
    message: `Brain Storm Entry made for project ID ${req.params.projectId}`,
  });
});
...
router.post('/:projectId/phase/:phaseNum', async (req, res) => {
  ...
  res.json({
    message: `Phase ${req.params.phaseNum} of project ${req.params.projectId} has been created`,
  });
});
...
router.post('/:projectId/phase/:phaseId/task/:taskId', async (req, res) => {
  ...
  const task = await prisma.task.create({
    data: {
      assignees: {
        connect: data.assignees.map(assignee => ({ id: assignee })),
      },
  ...
  res.json({
    message: `Task ${req.params.taskId} of phase ${req.params.phaseId} has been created`,
  });
});
...
router.put('/:projectId/phase/:phaseId/task/:taskId', async (req, res) => {
  ...
  res.json({
    message: `Updated task ${req.params.taskId} with the status ${data.status}`,
  });
});
